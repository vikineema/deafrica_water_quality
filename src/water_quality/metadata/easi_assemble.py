...
class EasiPrepare(Eo3Interface):
    ...
    def measurements(self) -> dict:
        return {
            name: (grid, path)
            for grid, name, path in self._measurements.iter_paths()
        }
    ...
    def _match_measurement_names_to_band_ids(
        ...
        for mtuple in mtuples:
            ...
            if len(common) == 1:
                measurement2path[mtuple[0]] = band_ids[
                    supplementary[common.pop()]
                ]
                continue
            raise RuntimeError(
        ...
        if self._dataset_scheme == "s3":
            ...
            if response["KeyCount"] > 0:
                for item in response["Contents"]:
                    ...
                    if m:
                        band_ids[m.group(1)] = (
                            f"s3://{self._dataset_bucket}/{key}"
                        )
                        continue

        if len(band_ids) == 0:
            raise RuntimeError(
                f"No matching file paths found for regex: {band_regex}"
            )
