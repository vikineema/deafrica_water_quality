...
def plot_change_in_permanent_water_area(
    ...
    if not lkrv_pwac:
        lkrv_pwac = permanent_water_area_change(
            ds,
            baseline_period,
            target_years,
        )

    # Get the permanent water body area for each year
    pixel_area = (
    ...
    da = da.where(~np.isinf(da), np.nan)
    ...
    _, fitted_y, *_ = piecewise_linreg(
        x=np.arange(0, da.time.size), y=da.values
    )
    interp_func = sp.interpolate.interp1d(
        x=da.time, y=fitted_y, kind="quadratic", fill_value="extrapolate"
    )

    # Generate a new, denser time range for interpolation
    time_new = pd.date_range(
        da.time.min().values, da.time.max().values, freq="2M"
    )
    # Fit a trend curve to the data
    trend_curve = np.clip(
        interp_func(time_new), da.min().item(), da.max().item()
    )

    # Statistics based on SDG method.
    baseline_slice = slice(min(baseline_period), max(baseline_period))
    baseline_times = da.sel(time=baseline_slice).time.values
    ...
    target_slice = slice(min(target_years), max(target_years))
    target_times = da.sel(time=target_slice).time.values
    ...
    plt.plot(
        ...
        linestyle="",
        marker="X",
        label="Permanent water area, mean for the waterbody (data points)",
    )
    plt.plot(
        time_new,
        trend_curve,
        "k",
        linestyle="dotted",
        label="Permanent water area, mean for the waterbody (smoothed fit)",
    )
    plt.plot(
        baseline_times,
        ...
        linestyle="dashed",
        marker="none",
        label="Permanent water area for baseline period (SDG method)",
    )
    plt.plot(
        target_times,
        target_years_mean,
        ...
        linestyle="dashed",
        marker="none",
        label="Permanent water area for target years (SDG method)",
    )
    plt.plot(
        regression_span,
        regression_line_y,
    ...
    plt.legend()
    plt.show()
    plt.savefig(f"{waterbody_uid}_lkrv_pwac.png", dpi=150)
