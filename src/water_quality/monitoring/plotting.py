...
def plot_change_in_trophic_state_tsi(
    waterbody_uid: str,
    ds: xr.Dataset,
    baseline_period: tuple[str],
    target_years: str,
    ...
    tsi_figure = get_figure_template()

    tsi_da = ds["TSI"].mean(dim=("x", "y"))
    tsi_da = tsi_da.where(~np.isinf(tsi_da), np.nan)

    # Data points plot
    label_prefix = "TSI values"
    data_points_plot = tsi_figure["data_points_plot"]
    data_points_plot.update(
        {
            "x": tsi_da.time.values,
            "y": tsi_da.values,
            "label": data_points_plot["label"].format(
                label_prefix=label_prefix
            ),
    ...
    # regression followed by a quadratic interpolator
    _, fitted_y, *_ = piecewise_linreg(
        x=np.arange(0, tsi_da.time.size),
        y=tsi_da.values,
    )
    interp_func = sp.interpolate.interp1d(
        x=tsi_da.time,
    ...
    # Generate a new, denser time range for interpolation
    time_new = pd.date_range(
        tsi_da.time.min().values,
        tsi_da.time.max().values,
        freq="2M",
    )
    # Fit a trend curve to the data
    baseline_slice = slice(min(baseline_period), max(baseline_period))
    baseline_times = tsi_da.sel(time=baseline_slice).time.values
    baseline_period_mean = (
        tsi_da.sel(time=baseline_times).mean(dim="time").item()
    )
    baseline_period_mean = np.round(baseline_period_mean, 1)

    target_slice = slice(min(target_years), max(target_years))
    target_times = tsi_da.sel(time=target_slice).time.values
    target_years_mean = tsi_da.sel(time=target_times).mean(dim="time").item()
    target_years_mean = np.round(target_years_mean, 1)
    ...
    trend_curve_plot = tsi_figure["trend_curve_plot"]
    trend_curve_plot.update(
        {
            "x": time_new,
            "y": trend_curve,
    ...
    sdg_baseline_period_plot = tsi_figure["sdg_baseline_period_plot"]
    sdg_baseline_period_plot.update(
        {
            "x": baseline_times,
            "y": baseline_period_mean_values,
    ...
    # SDG method target years plot
    target_years_mean_values = np.full(target_times.shape, target_years_mean)
    sdg_target_years_plot = tsi_figure["sdg_target_years_plot"]
    sdg_target_years_plot.update(
        {
            "x": target_times,
            "y": target_years_mean_values,
    ...
    # Robust regression line comparing baseline period and target years
    regression_span = tsi_da.sel(
        time=slice(min(baseline_times), max(target_times))
    ).time.values
    regression_line_x = regression_span.astype("float")
    regression_line_slope = lkw_qltrst["TSI_regression_slope"]
    regression_line_intercept = lkw_qltrst["TSI_regression_intercept"]
    regression_line_scale = 1
    regression_line_y = (
        (regression_line_slope * regression_line_x) + regression_line_intercept
    ) * regression_line_scale
    ...
    tsi_figure["figure_title"] = (
        ...
        f"\nSDG 'Affected' : {lkw_qltrst['affected']} \n"
        "Significant trend based on robust regression over baseline period and "
        f"target years = {lkw_qltrst['TSI_regression_significant']} \n"
        f"Decline in Chla (improvement) = {lkw_qltrst['TSI_declining']}"
    )

    tsi_figure["y_label"] = "Trophic State Index - TSI"
    tsi_figure["output_file_name"] = f"{waterbody_uid}_lkw_qltrst_TSI.png"
    plot_figure(tsi_figure)
